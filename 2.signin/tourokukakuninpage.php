...
// var_dump($_SESSION);
	...
	if(!isset($_SESSION['user_info'])){
		// 一度も入力せずに確認画面へ飛んだ場合は、登録画面へ戻す
		header('Location: shinkitoroku.php');
		exit();
	}
	...
	if(!empty($_POST)){
		...
		// echo 'ユーザ登録!';
		//DBにユーザ情報を登録する（永久的に保存）

		// 分かりやすくするためにセッションのデータを変数に入れる
	...
	$stmt->execute($data);
	// 登録後に完了ページへ飛ばす
	header('Location:tourokukannryoupage.php');
	exit();
...
<html lang="ja">
	...
	<a href="shinkitoroku.php">前の画面へ戻る</a><br>
